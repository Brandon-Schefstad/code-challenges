...
test('It should return only the strings that contain smiley faces', (assert) => {
	...
	assert.deepEqual(findHappiness(words), [
		'apple (:)',
		':)banana',
		'cant:)aloupe',
	]);
	...
	assert.deepEqual(findHappiness([':) yay', ':( no', '', '', '', '']), [
		':) yay',
	]);
});
...
test('It should return a standardized set of phone numbers', (assert) => {
	...
	assert.deepEqual(standardizePhoneNumbers(nums), [
		'1234567890',
		'2222222222',
	]);
...
test('It should find all the strings that contain a given string', (assert) => {
	...
	assert.deepEqual(findAnything(words, 'i'), [
		'things',
		'missing that thing',
	]);
});
...
test('It should sort events by the day on which they happen', (assert) => {
	...
	assert.equal(
		sortedEvents[0].includes('Dancing on Mondays and Tuesdays'),
		true
	);
	assert.equal(
		sortedEvents[0].includes('Meet the inventors! Monday, August 7'),
		true
	);
	assert.equal(
		sortedEvents[1].includes('Dancing on Mondays and Tuesdays'),
		true
	);
